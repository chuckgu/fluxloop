...
usage() {
  cat <<'EOF'
Usage: scripts/setup_fluxloop_env.sh [OPTIONS]
...
Run this script from the FluxLoop project directory (e.g. packages/cli).
EOF
}
...
if [[ -n "$TARGET_SOURCE_ROOT" ]]; then
  ...
  echo "➤ Writing VSCode workspace settings ($WORKSPACE_SETTINGS_FILE)"
...
JSON
  ...
  echo "➤ Skipping VSCode settings (no --target-source-root provided)"
fi
...
Setup complete! Next steps:
  1. Open this folder in VSCode (or reload window).
